...
public final class ResumeCommand extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(@NotNull final SlashCommandInteractionEvent event) {
        ...
        if (!scheduler.isPaused()) {
            event.reply(GuildLocalization.localizeString(guildID, LocalizationKey.NOT_PAUSED)).setEphemeral(true).queue();
            return;
        }
        scheduler.setPaused(false);
        event.reply(GuildLocalization.localizeString(guildID, LocalizationKey.TRACK_RESUMED)).queue();
    }
}
